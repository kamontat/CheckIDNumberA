...
class DefaultWorksheetFormat : WorksheetFormat<IDNumber, DefaultWorksheetFormat.PositionValue> {
    override fun getCellsInRow(id: Long, idNumber: IDNumber): Array<PositionValue> {
        return arrayOf(PositionValue(0, id.toString()), idNumber.getId()?.let { PositionValue(1, it) } ?: PositionValue(1, "-1"))
    }
